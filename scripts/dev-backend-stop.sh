...
MSSQL_DATA_DIR="${INFRA_DIR}/volumes/mssql"

if [[ "$CLEAN" == true ]]; then
    log "Stopping and removing DB container + wiping data directory (all data will be lost)..."
    $COMPOSE stop db
    $COMPOSE rm -f db
    rm -rf "$MSSQL_DATA_DIR" && log "Data directory removed: ${MSSQL_DATA_DIR}" || log "Data directory not found (already removed?)."
else
    log "Stopping SQL Server container (data preserved)..."
    $COMPOSE stop db
    log "Done. Data is safe in ${MSSQL_DATA_DIR}"
